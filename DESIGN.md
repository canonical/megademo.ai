...
**Project search.** Two JSON API endpoints provide filter-as-you-type search across project titles and descriptions using MongoDB `$regex` (case-insensitive, capped at 50 results). `GET /api/projects/search?q=&category=&team=&sort=` is public and returns only submitted/finalist projects. `GET /admin/projects/search?q=&status=&category=` is admin-only and searches all statuses. Both pages (`/projects` and `/admin/projects`) have a search input with 250 ms debounce; while a search term is active, dropdown filter changes re-query the API instead of submitting the form. Clearing the search restores the original server-rendered content.
